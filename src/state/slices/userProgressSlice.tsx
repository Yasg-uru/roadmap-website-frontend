...
>("userProgress/upsertUserProgress", async ({ userId, roadmapId, completedNodes }, thunkAPI) => {
  try {
    ...
    return thunkAPI.rejectWithValue(error.response?.data || "Failed to upsert user progress");
  }
});
...
const userProgressSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(upsertUserProgress.fulfilled, (state, action: PayloadAction<UserProgress>) => {
        state.progress = action.payload;
      });
  },
});
